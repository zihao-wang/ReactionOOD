...
class GOODRMGF(InMemoryDataset):
    r"""
    The GOOD-RMG-Family dataset. Adapted from 
    `Rmg database for chemical property prediction.`_.
    ...
    def get_domain_sorted_list(self, data_list, domain='family'):
        ...
        for data in tqdm(data_list):
            ...
            data.__setattr__(domain, data.family)

        # sorted_data_list = sorted(data_list, key=lambda data: getattr(data, domain))
        sorted_data_list = sorted(data_list, key=lambda data: data.family)
    ...
    def process(self):
        ...
        dataset = torch.load("/apdcephfs/share_1364275/yandrewchen/GOOD/datasets/rmgf.pt")[0].mol_graphs
        print('Load data done!')
        import csv
        csv_data = list(csv.reader(open("/apdcephfs/private_yandrewchen/GOOD2/GOOD/ReactionOOD/data/rmg/rmgf.csv","r")))
        families = {}
        for dd in csv_data:
            families[dd[2]] = len(families)
        data_list = []
        # print(len(csv_data),len(dataset))
        # assert len(csv_data)-1 == len(dataset)
        off_set = 0
        print(dataset[0].mol)
        for i, data in enumerate(dataset):
            ...
            # print(data.targets)
            cur_idx = i+off_set
            while csv_data[cur_idx+1][-2]!=data.mol:
                print(cur_idx+1,"\n",csv_data[cur_idx+1][-2],"\n",data.mol)
                # exit()
                off_set+=1
                cur_idx = i+off_set
                if cur_idx+1>=len(csv_data):
                    raise("GG")
            mol = data.mol.split(">")[0]
            edge_index = np.array(data.edge_list, dtype = np.int64).T
            A_value = np.log10(float(csv_data[i+1+off_set][3]))
            n_value= float(csv_data[i+1+off_set][4])
            ea_value= float(csv_data[i+1+off_set][5])/1000
            # target = torch.FloatTensor([A_value,n_value,ea_value]).view(-1,3)
            target = torch.FloatTensor([ea_value]).view(-1,1)
            ...
            data = Data(edge_index=torch.LongTensor(edge_index),
                                ...
                                idx=i,
                                mol=mol,
                                family=families[csv_data[i+1+off_set][2]]
                                )
