...
class GOODRMGT(InMemoryDataset):
    r"""
    The GOOD-RMG-T dataset. Adapted from
    ...
    def get_domain_sorted_list(self, data_list, domain='famaliy'):
    ...
    def process(self):
        dataset = torch.load(f"../data/ReactionOOD/rmgt.pt")[0].mol_graphs
        print('Load data done!')
        import csv
        csv_data = list(csv.reader(open(f"../data/rmg/AAM/rmgt.csv","r")))
        famalies = {}
        for dd in csv_data:
        ...
        for i, data in enumerate(dataset):
            ...
            data = Data(edge_index=torch.LongTensor(edge_index),
                                ...
                                torch.FloatTensor([float(csv_data[i+1+off_set][3])]).repeat(num_atoms).view(num_atoms,-1)],dim=-1),
                                y=torch.FloatTensor(data.targets).unsqueeze(1),
                                idx=i,
                                mol=mol,
                                domain=int(float(csv_data[i+1+off_set][3])))
