...
class GOODRMGP(InMemoryDataset):
    r"""
    The GOOD-RMG-TP dataset. Adapted from 
    ...
    def get_domain_sorted_list(self, data_list, domain='tp'):
        ...
        sorted_data_list = sorted(data_list, key=lambda data: data.domain)
    ...
    def process(self):
        dataset = torch.load(f"../data/ReactionOOD/rmgp.pt")[0].mol_graphs
        print('Load data done!')
        import csv
        csv_data = list(csv.reader(open(f"../data/rmg/AAM/rmgp.csv","r")))
        famalies = {}
        for dd in csv_data:
            famalies[dd[2]] = len(famalies)
        data_list = []
        ...
        for i, data in enumerate(dataset):
            ...
            while csv_data[cur_idx+1][-2]!=data.mol:
                ...
                if cur_idx+1>=len(csv_data):
                    raise("GG")
            domain = int(float(csv_data[i+1+off_set][3])*1e5+float(csv_data[i+1+off_set][4]))
            mol = data.mol.split(">")[0]
            edge_index = np.array(data.edge_list, dtype = np.int64).T
            num_atoms = len(data.f_atoms)
            data = Data(edge_index=torch.LongTensor(edge_index),
                                edge_attr=torch.tensor(data.edge_features),
                                x=torch.cat([torch.tensor(data.f_atoms),\
                                    torch.FloatTensor([float(csv_data[i+1+off_set][3]),float(csv_data[i+1+off_set][4]),\
                                    float(csv_data[i+1+off_set][3])*1e5+float(csv_data[i+1+off_set][4])]).repeat(num_atoms).view(num_atoms,-1)],dim=-1),
